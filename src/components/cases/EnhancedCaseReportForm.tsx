...
export const EnhancedCaseReportForm = () => {
  ...
  const onSubmit = async (data: CaseReportFormData) => {
    ...
    try {
      ...
      let assigned_handler: 'tppk' | 'p4gn' | 'arps' | null = null;
      if (data.category === 'narkoba') {
        assigned_handler = 'p4gn';
      } else if (data.category === 'kekerasan' || data.category === 'bullying') {
        assigned_handler = 'tppk';
      } else if (data.category === 'pergaulan_bebas') {
        assigned_handler = 'arps';
      } else {
        assigned_handler = 'tppk'; // Default to TPPK for other cases
      }

      // Determine status based on immediate action
      let status: 'pending' | 'under_review' = data.immediate_action_needed ? 'under_review' : 'pending';

      // Prepare the case data with proper typing
      const caseData = {
        case_number: '', // Will be auto-generated by trigger
        title: data.title,
        description: data.description,
        category: data.category,
        priority: data.immediate_action_needed ? 'critical' as const : data.priority,
  ...
  const getCategoryInfo = (category: string) => {
    const categoryInfo = {
      ...
      pergaulan_bebas: { label: 'Pergaulan Bebas', color: 'bg-pink-100 text-pink-800', handler: 'ARPS' },
